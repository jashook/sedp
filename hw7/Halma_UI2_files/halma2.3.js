...
function makeMove() {
    ...
    var resp = makeAjaxPostMoveRequestNoParm(currentTeam);
    var move;
    try {
    ...
    document.getElementById("responseString").innerHTML = JSON.stringify(move);
     ...
     if(!isCellOnTeam(currPieceLoc, gTeamList[currentTeam].teamPieces)) {
       ...
       if (!isValidMoveRequest(currPieceLoc, moves, gPieces) ){
        ...
        // set current Piece position to last entry in move request list
...
// jumpArr has original source piece followed by jump locations
